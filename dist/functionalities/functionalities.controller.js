"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.FunctionalitiesController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const functionality_entity_1 = require("./entities/functionality.entity");
const functionalities_service_1 = require("./functionalities.service");
let FunctionalitiesController = exports.FunctionalitiesController = class FunctionalitiesController {
    constructor(functionalitiesService) {
        this.functionalitiesService = functionalitiesService;
    }
    async findAll(role_id) {
        return await this.functionalitiesService.findAll(role_id);
    }
};
__decorate([
    (0, common_1.Get)(),
    (0, common_1.HttpCode)(common_1.HttpStatus.OK),
    (0, swagger_1.ApiOperation)({ summary: 'Get functionalities list' }),
    (0, swagger_1.ApiResponse)({
        status: common_1.HttpStatus.OK,
        description: 'Get functionalities list',
        type: [functionality_entity_1.Functionality],
    }),
    (0, swagger_1.ApiQuery)({ name: 'role_id', required: false, type: Number }),
    __param(0, (0, common_1.Query)('role_id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number]),
    __metadata("design:returntype", Promise)
], FunctionalitiesController.prototype, "findAll", null);
exports.FunctionalitiesController = FunctionalitiesController = __decorate([
    (0, swagger_1.ApiTags)('Functionalities'),
    (0, common_1.Controller)({
        path: 'functionalities',
        version: '1',
    }),
    __metadata("design:paramtypes", [functionalities_service_1.FunctionalitiesService])
], FunctionalitiesController);
//# sourceMappingURL=functionalities.controller.js.map